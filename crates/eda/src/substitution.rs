...
impl EdaSubstitutionRule {
    ...
    pub fn apply(&self, eda_placement: &EdaPlacement) -> EdaPlacement {
        let result = self
            .transforms
            .iter()
            .fold(eda_placement.clone(), |mut placement, change_item| {
                if let Some(field) = placement
                    .fields
                    .iter_mut()
                    .find(|field| {
                        field
                            .name
                            .eq(change_item.field_name.as_str())
                    })
                {
                    field
                        .value
                        .clone_from(&change_item.field_value);
                }
...
impl EdaSubstitutor {
    pub fn substitute<'placement, 'rule>(
        original_eda_placements: &'placement [EdaPlacement],
        eda_substitution_rules: &'rule [EdaSubstitutionRule],
    ) -> Vec<EdaSubstitutionResult<'placement, 'rule>> {
        let mut results = vec![];
            ...
            loop {
                ...
                for rule in eda_substitution_rules.iter() {
                    if rule.matches(&eda_placement) {
                        let original_eda_placement = eda_placement.clone();
                        eda_placement = rule.apply(&original_eda_placement);

                        // if the rule makes no modification to the placement, then it will match again on the next iteration causing an infinite loop which we must avoid
                        // a perfectly valid scenario is where the value matches any value the value is transformed to an empty value
                        // e.g. name_pattern="SOME_MPN", name="SOME_MPN", value_pattern=/.*/, value=""

                        if eda_placement.ne(&original_eda_placement) {
                            applied_rule_count_this_pass += 1;
                            chain.push(EdaSubstitutionChainEntry {
                                rule,
                            });
                        }
                    }
...
pub mod eda_substitutor_tests {
    ...
    use crate::substitution::{
        EdaSubstitutionChainEntry, EdaSubstitutionResult, EdaSubstitutionRule, EdaSubstitutionRuleTransformItem,
        EdaSubstitutor,
    };
    ...
    pub fn substitute_one_diptrace_placement_using_a_chain() {
        ...
        let first_eda_substitution_rule = EdaSubstitutionRule {
            ...
            transforms: vec![
                EdaSubstitutionRuleTransformItem {
                    field_name: "name".to_string(),
                    field_value: "INTERMEDIATE_NAME1".to_string(),
                },
                EdaSubstitutionRuleTransformItem {
                    field_name: "value".to_string(),
        ...
        let second_eda_substitution_rule = EdaSubstitutionRule {
            criteria: vec![
                Box::new(ExactMatchCriterion {
                    field_name: "name".to_string(),
                    field_pattern: "INTERMEDIATE_NAME1".to_string(),
                }),
                Box::new(ExactMatchCriterion {
                    field_name: "value".to_string(),
                    field_pattern: "INTERMEDIATE_VALUE1".to_string(),
                }),
            ],
            transforms: vec![
                EdaSubstitutionRuleTransformItem {
        ...
        // and
        let expected_results = vec![EdaSubstitutionResult {
            ...
            chain: vec![
                EdaSubstitutionChainEntry {
                    rule: &eda_substitution_rules[1],
                },
                EdaSubstitutionChainEntry {
                    rule: &eda_substitution_rules[0],
                },
            ],
        }];

        // when
        let results = EdaSubstitutor::substitute(eda_placements.as_slice(), eda_substitution_rules.as_slice());
    ...
    pub fn substitute_one_kicad_placement_using_a_chain() {
        ...
        let first_eda_substitution = EdaSubstitutionRule {
            criteria: vec![
                Box::new(ExactMatchCriterion {
                    ...
                    field_pattern: "PACKAGE1".to_string(),
                }),
                Box::new(ExactMatchCriterion {
                    field_name: "val".to_string(),
                    field_pattern: "VAL1".to_string(),
                }),
            ],
            transforms: vec![
                EdaSubstitutionRuleTransformItem {
                    field_name: "package".to_string(),
                    field_value: "INTERMEDIATE_PACKAGE1".to_string(),
                },
                EdaSubstitutionRuleTransformItem {
                    field_name: "val".to_string(),
        ...
        let second_eda_substitution = EdaSubstitutionRule {
            criteria: vec![
                Box::new(ExactMatchCriterion {
                    field_name: "package".to_string(),
                    field_pattern: "INTERMEDIATE_PACKAGE1".to_string(),
                }),
                Box::new(ExactMatchCriterion {
                    field_name: "val".to_string(),
                    field_pattern: "INTERMEDIATE_VAL1".to_string(),
                }),
            ],
            transforms: vec![
                EdaSubstitutionRuleTransformItem {
                    field_name: "package".to_string(),
                    field_value: "SUBSTITUTED_PACKAGE1".to_string(),
                },
                EdaSubstitutionRuleTransformItem {
                    field_name: "val".to_string(),
        ...
        let expected_results = vec![EdaSubstitutionResult {
            ...
            chain: vec![
                EdaSubstitutionChainEntry {
                    rule: &eda_substitutions[1],
                },
                EdaSubstitutionChainEntry {
                    rule: &eda_substitutions[0],
                },
            ],
        }];

        // when
        let results = EdaSubstitutor::substitute(eda_placements.as_slice(), eda_substitutions.as_slice());
    ...
    #[test]
    pub fn substitute_using_rule_that_always_matches() {
        // given
        let eda_placement1 = EdaPlacement {
            ref_des: "R1".to_string(),
            fields: vec![
        ...
        // Note: The 'name' pattern and 'value' patterns must always match even after the transforms are applied.
        let eda_substitution_rule = EdaSubstitutionRule {
            criteria: vec![
                Box::new(ExactMatchCriterion {
                    field_name: "name".to_string(),
                    field_pattern: "NAME1".to_string(),
        ...
        let expected_results = vec![EdaSubstitutionResult {
            ...
            chain: vec![EdaSubstitutionChainEntry {
                rule: &eda_substitution_rules[0],
            }],
        }];
