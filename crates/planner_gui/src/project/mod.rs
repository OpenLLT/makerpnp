...
impl Project {
    ...
    pub fn make_widget(&mut self) -> WidgetInstance {
        ...
        let default_content = "content-pane"
            ...
            .centered()
            .and(
                self.project_tree_path
                    .to_label()
                    .centered(),
            )
            .into_rows();

        let default_content_handle = self.pile.push(default_content);
        ...
        let action = match message {
            ...
            ProjectMessage::RequestView(view) => {
                let event = match view {
                    ...
                    ProjectViewRequest::PhaseOverview {
                        ...
                        phase_reference: Reference(phase),
                    },
                ...
                let task = self.core_service.update(event);
                ProjectAction::Task(task)
            }
            ProjectMessage::Navigate(path) => {
                ...
                if let Some(captures) = phase_pattern.captures(&path) {
                    ...
                    let tasks: Vec<_> = vec![
                        Task::done(ProjectMessage::RequestView(ProjectViewRequest::PhaseOverview {
                            phase: phase_reference.clone(),
                        })),
                        Task::done(ProjectMessage::RequestView(ProjectViewRequest::PhasePlacements {
                            phase: phase_reference.clone(),
                        })),
                    ];
            ...
            ProjectMessage::Error(error) => ProjectAction::ShowError(error),
            ProjectMessage::UpdateView(view) => {
                ...
                match view {
                    ProjectView::Overview(project_overview) => {
                        ...
                        self.name
                            .set(Some(project_overview.name.clone()));
                    ...
                    ProjectView::Placements(placements) => ProjectAction::None,
    ...
    fn update_phase_state(
        ...
        mut phase_overview: Option<PhaseOverview>,
        mut phase_placements: Option<PhasePlacements>,
    ) {
                    ...
                    DfsEvent::Discover(node, _) => {
                        ...
                        let node_widget = title
                            ...
                            .on_click(move |_event| {
                                sender
                                    .send(ProjectMessage::Navigate(path.clone()))
                                    .expect("sent");
                            })
                            .make_widget();

                        let child_key = project_tree
                            .insert_child(node_widget, current_parent_key.as_ref())
                            .unwrap();
                ...
                move || {
                    ...
                    match validations.is_valid() {
                        ...
                        true => {
                            ...
                            sender
                                .send(ProjectMessage::AddPcb(args))
                                .expect("sent");
...
fn make_phase_overview_widget(dyn_overview: &Dynamic<PhaseOverview>) -> impl MakeWidget + Sized {
    ...
    let properties = Properties::default()
        ...
        .with_footer_label(
            Localize::new("phase-properties-footer").arg("count", FluentValue::Number(FluentNumber::from(items.len()))),
        )
        .with_items(items);
...
fn update_stuff(
    dyn_overview: &Option<Dynamic<PhaseOverview>>,
    dyn_placements: &Option<Dynamic<PhasePlacements>>,
) -> WidgetInstance {
