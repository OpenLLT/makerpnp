...
impl egui_table::TableDelegate for PlacementStateTable<'_> {
    fn header_cell_ui(&mut self, ui: &mut Ui, cell: &HeaderCellInfo) {
        ...
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                let (l_resp, r_resp) = egui::Sides::new()
                    ...
                    .show(
                        ...
                        |ui| {
                            let label = match cell.group_index {
                                ...
                                1 => Label::new(RichText::new(tr!("table-placements-column-object-path")).strong()).selectable(false),
                                2 => Label::new(RichText::new(tr!("table-placements-column-refdes")).strong()).selectable(false),
                                3 => Label::new(RichText::new(tr!("table-placements-column-placed")).strong()).selectable(false),
                                4 => Label::new(RichText::new(tr!("table-placements-column-manufacturer")).strong()).selectable(false),
                                5 => Label::new(RichText::new(tr!("table-placements-column-mpn")).strong()).selectable(false),
                                6 => Label::new(RichText::new(tr!("table-placements-column-rotation")).strong()).selectable(false),
                                7 => Label::new(RichText::new(tr!("table-placements-column-x")).strong()).selectable(false),
                                8 => Label::new(RichText::new(tr!("table-placements-column-y")).strong()).selectable(false),
                                9 => Label::new(RichText::new(tr!("table-placements-column-pcb-side")).strong()).selectable(false),
                                _ => unreachable!(),
                            };
                        ...
                        |ui| {
                            if let Some(pos) = self.state.sort.iter().position(|(c, ..)| c == &col) {
                ...
                // FIXME we want to detect a click on the space between the sides and the sides themselves
                //       currently we can't detect a click on the space between the sides.
                if l_resp.clicked() || r_resp.clicked() {
